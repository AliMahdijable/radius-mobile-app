...
///
/// **٣) حقلان يُحذفان قبل الكتابة — كلمة المرور وحالة الاتّصال.**
///
/// `password`: القائمة تحملها من الساس، والإجابة عن «كم عليّ؟» لا
/// تحتاجها. وحذفُ أخطر حقلٍ أقوى من تشفيره.
///
/// و`is_online`/`online_status`: **لا عمر مقبول لها إطلاقاً.** جلسة
/// PPPoE في العراق تسقط مع كلّ قطع كهرباء، فنقطةٌ خضراء عمرها ساعة
/// كاذبةٌ غالباً وعمرها يوم كاذبةٌ يقيناً. والضرر ليس في الرقم بل في
/// القرار: المدير يرى «متّصل» فيقول لزبونه «الخطّ شغّال، المشكلة
/// عندك» ولا يُرسل فنّيّاً. والبانر يقول «الأرقام قد تكون تغيّرت»
/// فيقرؤه المدير على الدين ويصدّق النقطة.
///
/// ⚠️ وهذا **إلغاءٌ لا تخفيف**، كما في المبدأ (١): ما لا يُكتب لا
/// يكذب. و`fromJson` تُنتج `isOnlineFlag: false` حتماً حين تغيب
/// الحقول، والنداء الحيّ `loadOnline` يملؤها فور عودة الشبكة.
///
/// ── وأين يُكتب ──────────────────────────────────────────────────
...
class SubscribersOfflineCache {
  ...
  /// الحقول التي لا تنزل للقرص. والخادم يطلب `is_online` و
  /// `online_status` من الساس صراحةً ضمن أعمدة `/api/v2/subscribers`،
  /// فهي موجودةٌ في الصفوف فعلاً — وبلا هذا الحذف تُرسَم نقطةٌ خضراء
  /// عمرها ثلاثة أيّام.
  static const _drop = ['password', 'is_online', 'online_status', 'isOnline'];
