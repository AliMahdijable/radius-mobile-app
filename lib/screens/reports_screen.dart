...
class _ReportsScreenState extends ConsumerState<ReportsScreen> {
  ...
  void initState() {
    ...
    _dateFrom = intl.DateFormat('yyyy-MM-dd').format(last30);
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text('فلتر التاريخ', style: Theme.of(ctx).textTheme.titleMedium
                  ...
                  Row(children: [
                    Expanded(child: _DateBtn(
                      ...
                      onTap: () async {
                        final d = await showDatePicker(
                          context: ctx,
                          initialDate: DateTime.tryParse(from) ?? DateTime.now(),
                          firstDate: DateTime(2020),
                          lastDate: DateTime.now(),
                        );
                        if (d != null) {
                          setSheet(() => from = intl.DateFormat('yyyy-MM-dd').format(d));
                        }
                      },
                    )),
                    ...
                    Expanded(child: _DateBtn(
                      ...
                      onTap: () async {
                        final d = await showDatePicker(
                          context: ctx,
                          initialDate: DateTime.tryParse(to) ?? DateTime.now(),
                          firstDate: DateTime(2020),
                          lastDate: DateTime.now().add(const Duration(days: 1)),
                        );
                        if (d != null) {
                          setSheet(() => to = intl.DateFormat('yyyy-MM-dd').format(d));
                        }
                      },
                    )),
                  ]),
                  ...
                  Wrap(spacing: 8, children: [
                    _QuickDateChip('اليوم', () {
                      ...
                      setSheet(() { from = today; to = today; });
                    }),
                    _QuickDateChip('آخر 7 أيام', () {
                      final now = DateTime.now();
                      setSheet(() {
...
class _ActivityRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final style = _ReportsScreenState._getActionStyle(activity);
              ...
              children: [
                Row(children: [
                  ...
                  Text(formattedTime, style: TextStyle(
                    fontSize: 10, color: theme.colorScheme.onSurface.withOpacity(0.4))),
                ]),
                ...
                if (admin.isNotEmpty)
                  Padding(
                    padding: const EdgeInsets.only(top: 2),
                    child: Text('المدير: $admin', style: TextStyle(
                      fontSize: 10,
...
class _QuickDateChip extends StatelessWidget {
  final String label;
  final VoidCallback onTap;
  const _QuickDateChip(this.label, this.onTap);
